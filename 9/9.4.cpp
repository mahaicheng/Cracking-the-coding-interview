...
vector<vector<int>> subSet(const vector<int>& set) {
  ...
  for (auto iter = lastSetOfSet.begin(); iter != lastSetOfSet.end();
       ++iter) {  // for every vector set ,append a new element lastInt
...
int main() {
  vector<int> iVec{4, 5, 8, 1, 3, 4,  7,   1,  2,  6,
                   8, 9, 0, 4, 3, 32, 321, 55, 97, 325};
